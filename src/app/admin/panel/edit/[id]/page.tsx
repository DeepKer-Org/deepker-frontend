...
const EditUser = () => {
  ...
  const [initialDoctorData, setInitialDoctorData] =
    useState<DoctorFormValues | null>(null);
  const [initialAdminData, setInitialAdminData] =
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        enqueueSnackbar("Error al obtener la información del usuario.", {
          variant: "error",
        });
      }
    };
  ...
  const handleAdminSubmit = async (values: AdminFormValues) => {
    ...
    try {
      ...
      enqueueSnackbar("¡Se ha actualizado el usuario exitosamente!", {
        variant: "success",
      });
      router.back();
    } catch {
  ...
  const handleDoctorSubmit = async (values: DoctorFormValues) => {
    ...
    try {
      ...
      enqueueSnackbar("Error updating especialista médico.", {
        variant: "error",
      });
    }
  };
